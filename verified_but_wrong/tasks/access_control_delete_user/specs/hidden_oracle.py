...
def run_tests(candidate_func) -> dict:
    ...
    tests.append(_record("member_cannot_delete_target_counterexample", member.get("status") == "error", "status=error for member actor", f"status={member.get('status')}"))

    guest = candidate_func(_actor("guest", "u_guest"), _target())
    tests.append(_record("guest_cannot_delete_target_counterexample", guest.get("status") == "error", "status=error for guest actor", f"status={guest.get('status')}"))

    self_delete = candidate_func(_actor("admin", "u_admin"), _target(user_id="u_admin"))
    tests.append(_record("self_delete_counterexample", self_delete.get("status") == "error", "status=error for admin deleting self", f"status={self_delete.get('status')}"))

    missing = candidate_func(_actor(), None)
    tests.append(_record("missing_target_counterexample", missing.get("status") == "error", "status=error for missing target", f"status={missing.get('status')}"))

    deleted = candidate_func(_actor(), _target(status="deleted"))
    tests.append(_record("deleted_target_cannot_be_deleted_again", deleted.get("status") == "error", "status=error for deleted target", f"status={deleted.get('status')}"))

    admin_user_role = candidate_func(_actor("admin_user", "u_weird"), _target())
    tests.append(_record("role_must_be_exact_admin", admin_user_role.get("status") == "error", "status=error for role admin_user", f"status={admin_user_role.get('status')}"))
