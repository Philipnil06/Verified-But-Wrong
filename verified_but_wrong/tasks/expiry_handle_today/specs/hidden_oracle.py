...
def run_tests(candidate_func) -> dict:
    ...
    tests.append(_record("expired_yesterday_counterexample", expired.get("status") == "ok" and expired.get("handle_today") is True, "status=ok, handle_today=True", f"status={expired.get('status')}, handle_today={expired.get('handle_today')}"))

    lead_two = candidate_func(_item("2026-05-12", 3), "2026-05-10")
    tests.append(_record("lead_time_counterexample", lead_two.get("status") == "ok" and lead_two.get("handle_today") is True, "status=ok, handle_today=True for expiry in 2 days with lead 3", f"status={lead_two.get('status')}, handle_today={lead_two.get('handle_today')}"))

    boundary = candidate_func(_item("2026-05-13", 3), "2026-05-10")
    tests.append(_record("exact_boundary_counterexample", boundary.get("status") == "ok" and boundary.get("handle_today") is True, "status=ok, handle_today=True when handle date equals today", f"status={boundary.get('status')}, handle_today={boundary.get('handle_today')}"))
    ...
    tests.append(_record("invalid_date_counterexample", invalid.get("status") == "error", "status=error for invalid date", f"status={invalid.get('status')}"))

    negative_lead = candidate_func(_item("2026-05-12", -1), "2026-05-10")
    tests.append(_record("negative_lead_time_counterexample", negative_lead.get("status") == "error", "status=error for negative lead time", f"status={negative_lead.get('status')}"))
