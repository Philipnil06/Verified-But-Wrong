...
def run_tests(fn):
    tests=[]; out=fn(user(), sessions(), 'member'); tests.append(res('privileged_sessions_invalidated', out['status']=='ok' and not out['sessions'][0]['active'] and out['sessions'][1]['active'],'u1 privileged inactive only',str(out)))
