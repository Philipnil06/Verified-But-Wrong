...
def run_tests(candidate_func) -> dict:
    ...
    tests.append(_record("valid_refund_within_window_succeeds", valid.get("status") == "ok", "status=ok", f"status={valid.get('status')}"))

    over = candidate_func(_order(refunded_total=950.0), 100.0, "2026-04-10")
    tests.append(_record("refund_exceeding_remaining_amount_fails", over.get("status") == "error", "status=error", f"status={over.get('status')}"))

    tests.append(_record("refund_updates_refunded_total", valid_order.get("refunded_total") == 100.0, "refunded_total=100.0", f"refunded_total={valid_order.get('refunded_total')}"))
    ...
    tests.append(_record("day_31_without_override_fails", day31.get("status") == "error", "status=error for purchase 2026-04-01, current 2026-05-02, no override", f"status={day31.get('status')}"))
    ...
    negative = candidate_func(_order(), -1.0, "2026-04-10")
