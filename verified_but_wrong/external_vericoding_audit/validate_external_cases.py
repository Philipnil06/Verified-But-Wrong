...
def discover_manual_cases(manifest: Optional[Path]) -> List[Dict[str, Any]]:
    ...
    for e in entries:
        uniq[str(e.get("case_dir") or e.get("id"))] = e
...
def run_case(entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if weak_path.exists():
            ...
            if weak_pass is not True:
                demo_type = "manual_only"
...
def generate_paper_insert(validation_result: Dict[str, Any]) -> None:
    ...
    if len(demos) < 3:
        text = "\n".join([
            ...
            "",
            "This external audit should not be presented as paper-ready until at least 3 manually defensible cases are validated.",
        ])
    else:
        rows = []
        for c in demos:
        ...
        text = "\n".join([
            ...
            "",
            markdown_table(["Case", "Source", "Gap category", "NL intent", "Formal spec gap", "Bad candidate", "Demo type", "Adapted spec check", "Intended examples", "VBW demo"], rows),
            "",
            "## Limitations",
            "",
            "- Manual validation.",
...
def render_markdown(result: Dict[str, Any]) -> str:
    if result.get("status") == "no_validated_cases":
        return "# External Vericoding Validated Cases\n\nNo manually validated cases were found.\n"
    rows = []
    for c in result.get("validated_cases", []):
        rows.append([c.get("id"), c.get("source", ""), c.get("gap_category", ""), c.get("demo_type", ""), c.get("weak_spec_proxy_pass"), c.get("intended_oracle_pass"), c.get("verified_but_wrong_demo")])
    md = ["# External Vericoding Validated Cases", "", markdown_table(["Case", "Source", "Gap category", "Demo type", "Adapted spec check", "Intended examples", "VBW demo"], rows), ""]
    for c in result.get("validated_cases", []):
...
def main() -> int:
    ...
    if not entries:
        result = {"status": "no_validated_cases", "validated_case_count": 0, "verified_but_wrong_demo_count": 0, "paper_ready": False, "validated_cases": []}
