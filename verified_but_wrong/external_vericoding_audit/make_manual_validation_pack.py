...
def write_case(case_dir: Path, c: Dict[str, Any]) -> Dict[str, Any]:
    ...
    existing_manual = case_dir / "manual_validation.json"
    if existing_manual.exists():
        try:
            manual = json.loads(existing_manual.read_text(encoding="utf-8"))
            if manual.get("status") == "validated":
                generated = [p.name for p in case_dir.iterdir() if p.is_file()]
                return {"id": str(c.get("id")), "case_dir": str(case_dir), "status": "preserved_validated_case", "generated_files": sorted(generated)}
        except Exception:
            pass
