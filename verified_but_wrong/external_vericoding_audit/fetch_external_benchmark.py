...
def with_meta(status: str, dataset: Dict[str, Any] | None, attempts: list[Dict[str, Any]], reason: str | None = None) -> Dict[str, Any]:
    out: Dict[str, Any] = {"status": status, "timestamp": int(time.time()), "attempts": attempts}
...
def main() -> int:
    ...
    try:
        if args.jsonl_path:
            ...
            result = with_meta("available", register_jsonl_path(args.jsonl_path), attempts)
        ...
        if args.local_path:
            ...
            result = with_meta("available", dataset, attempts)
            write_json(REPORTS / "external_vericoding_fetch.json", result)
            write_text(REPORTS / "external_vericoding_fetch.md", render_fetch_md(result))
            print(json.dumps(result, indent=2))
            return 0
        ...
        if existing and not args.refresh:
            result = with_meta("available", meta, attempts)
            write_json(REPORTS / "external_vericoding_fetch.json", result)
            write_text(REPORTS / "external_vericoding_fetch.md", render_fetch_md(result))
            print(json.dumps(result, indent=2))
            return 0
        ...
        for variant in variants_for_repo_arg(args.repo):
            ok, dmeta, msg = clone_variant(variant, refresh=args.refresh)
            attempts.append({"repo_name": variant.repo_name, "repo_url": variant.repo_url, "method": "git", "ok": ok, "message": msg})
            if ok:
                result = with_meta("available", dmeta, attempts)
                write_json(REPORTS / "external_vericoding_fetch.json", result)
                write_text(REPORTS / "external_vericoding_fetch.md", render_fetch_md(result))
                print(json.dumps(result, indent=2))
                return 0
        ...
        if not args.no_raw_fallback:
            for variant in variants_for_repo_arg(args.repo):
                ok, dmeta, msg = raw_download_variant(variant)
                attempts.append({"repo_name": variant.repo_name, "repo_url": variant.repo_url, "method": "raw", "ok": ok, "message": msg})
                if ok:
                    result = with_meta("available", dmeta, attempts)
                    write_json(REPORTS / "external_vericoding_fetch.json", result)
                    write_text(REPORTS / "external_vericoding_fetch.md", render_fetch_md(result))
    ...
    result = with_meta("skipped", None, attempts, reason=reason)
    write_json(REPORTS / "external_vericoding_fetch.json", result)
    write_text(REPORTS / "external_vericoding_fetch.md", render_fetch_md(result))
