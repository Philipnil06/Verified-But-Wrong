    ...
    if old_role == "admin" and new_role != "admin":
        for session in updated.get("sessions", {}).values():
            if session.get("user_id") == user_id:
                ...
                session["claims"]["role"] = new_role
    return {"status": "ok", "message": "role updated and stale privileged sessions neutralized", "state": updated}
...
def perform_admin_action(state: dict, session_id: str) -> dict:
    ...
    session_role = session.get("claims", {}).get("role")
    if current_role == "admin" and session_role == "admin":
        return {"status": "ok", "allowed": True, "message": "admin action allowed"}
    return {"status": "error", "allowed": False, "message": "stale or non-admin session denied"}
